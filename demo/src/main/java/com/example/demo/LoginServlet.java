...
// Вывести форму
        pw.println("<form action='/demo_war_exploded/' method='post'>Write your name 4321: <input type='text' name='name' value=''><input type='submit' value='Enter chat'>");
...
// Переадресовать обратно на исходную страницу с формой
            response.sendRedirect(response.encodeRedirectURL("/demo_war_exploded/"));
        }
    }
...
// Определить идентификатор Java-сессии пользователя
        ...
        //System.out.println("LoginServlet java-session");
...
// нового пользователя в список активных
            ...
            ///отправить от чат-бота сообщение о входе нового пользователя
            ChatUser SystemBot = activeUsers.get("System");
            String SystemMessage = name+" invited for server";
            synchronized (messages)
            {
                // Добавить в список сообщений новое
                messages.add(new ChatMessage(SystemMessage, SystemBot, Calendar.getInstance().getTimeInMillis()));
            }
...
// Извиниться, отказать и попросить ввести другое имя
            return "Sorry, but name <strong>" + name + "</strong> is already taken. Please choose another one!";
        }
    }
}
